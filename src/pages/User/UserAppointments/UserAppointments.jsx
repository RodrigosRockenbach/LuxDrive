...
import { getAppointmentsByClient, cancelAppointment } from "../../../services/appointmentService";
...
export default function UserAppointments() {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      ...
      try {
        ...
        setMessage(err.message || "Erro ao buscar agendamentos.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleCancelAppointment = async (appointment) => {
    try {
      await cancelAppointment(appointment.id, appointment.date, 30);
              ...
              <div key={app.id} className="col-md-4">
                <div className="border rounded p-3 bg-white shadow-sm h-100">
                  ...
                  {isBefore(now, subMinutes(new Date(app.date), 30)) && (
                    <button className="btn btn-danger mt-2 w-100" onClick={() => handleCancelAppointment(app)}>
